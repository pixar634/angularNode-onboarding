...
export const connect = async config => {
    ...
    const url = `${database.protocol}+srv://${(database.user) ? encodeURIComponent(database.user) : ''}${(database.pwd) ? ':' : ''}${(database.pwd) ? encodeURIComponent(database.pwd) : ''}${(database.pwd) ? '@' : ''}${database.host}/${database.database}?retryWrites=true`;
